...
class Photo2ShapeDialog( QDialog, Ui_Photo2ShapeDialog ):
  ...
  def selectInputDir( self ):
    inputDir = QFileDialog.getExistingDirectory( self, "Select directory with images" )
    if inputDir.isEmpty():
      return
  ...
  def selectOutputFile( self ):
    ...
    settings = QSettings()
    lastDir = settings.value( "/UI/lstShapefileDir" ).toString()
  ...
  def accept( self ):
    ...
    shapeFileWriter = QgsVectorFileWriter( outFileName, self.outputEncoding, shapeFields, QGis.WKBPoint, None )

    featureId = 0
    #baseDir = str( self.inputDirEdit.text() )
    baseDir = self.inputDirEdit.text()

    QApplication.setOverrideCursor( QCursor( Qt.WaitCursor ) )
    self.okButton.setEnabled( False )
    for fileName in self.inputFiles:
      #filePath = os.path.join( baseDir, str( fileName ) )
      filePath = QFileInfo( baseDir + "/" + fileName ).absoluteFilePath()
      imgFile = open( str( filePath ), "rb" )
      exifTags = EXIF.process_file( imgFile, details = False )
      imgFile.close()
      ...
      feature.addAttribute( 0, filePath )
      ...
      self.progressBar.setValue( featureId )
